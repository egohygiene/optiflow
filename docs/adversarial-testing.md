...
| External process hangs, cancellation, non-zero exit, invalid JSON, or excessive output | The child is terminated and the caller receives a typed bounded error | `subprocess::tests` | Two-second test timeout, 1 KiB stdout/stderr limits, two-process concurrency limit |
